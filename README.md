...
- サーバーレス関数の話らしい.ブログには使わない
- ラムダについて何もしらん　APIアクセスで直接関数を叩けるみたいなイメージ？
- サーバーレスについても学びが必要そう。
- 例として、フォームの入力処理が挙げられていた。フォームの情報送信先をAPIルートにする。
  - 受け取った値をDBなりに保存する処理をAPIルート側に書くイメージ。
- 要はサーバーサイドの処理を書く場所のはず
- ルーティングと同じく動的にできる　多分[]を書く

## next.jsアプリのデプロイ

- ヘッドレスという単語がよく出るのであとで調べる
- github必須らしい　npm叩くフォルダより1段上にgitがあるのでひっかかるかも
- vercelにとりあえずデプロイできた　ルートディレクトリの設定が初期だとおかしくて404になってた
